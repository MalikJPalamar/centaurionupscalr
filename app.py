...
async def upscale():
    app.logger.info('Received upscale request')
    data = request.json
    filenames = data.get('filenames')
    if not filenames:
        app.logger.error('No filenames provided for upscaling')
    ...
    app.logger.info(f'Successfully upscaled {len(upscaled_files)} files')
...
async def analyze_image_async(original_path, upscaled_path):
    try:
        ...
        app.logger.error(f"Error analyzing {original_path} and {upscaled_path}: {str(e)}")
        return None
...
async def analyze():
    ...
    if not original_filenames or not upscaled_filenames or len(original_filenames) != len(upscaled_filenames):
        app.logger.error('Invalid filenames provided for analysis')
    ...
    for original_filename, upscaled_filename in zip(original_filenames, upscaled_filenames):
        ...
        if not os.path.exists(original_path) or not os.path.exists(upscaled_path):
            app.logger.error(f'One or both files not found: {original_filename}, {upscaled_filename}')
    ...
    app.logger.info(f'Successfully analyzed {len(analysis_results)} image pairs')
